    ...
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        <div class="flex items-center">
            <div class="text-base mr-auto">
                <span class="font-semibold mr-3">{{__('Sales By Agency')}}</span>
            </div>
        </div>

        <div class="mt-12 mx-auto h-full">
            <pdf-viewer v-slot="{ src, offices, getSalesByAgencyPdfData, viewer_show, loading }" :get-offices="{{$offices}}"  search-url="{{ route('sales-by-agency.print') }}">
                <div class="grid grid-cols-5 gap-3">
                    {{-- <div class="col-span-1 sm:col-span-1"> --}}
                        {{-- <div class="grid grid-cols-2 gap-3"> --}}

                            <div class="col-span-1 sm:col-span-1">
                                <x-label for="terminal_id" class="font-semibold">Terminal</x-label>
                                <multi-select :items="{{$terminals}}" v-slot="{ selected }" name="terminal_id" label="name" search-by="departure_city_id" type="state"></multi-select>
                            </div>

                            <div class="col-span-1 sm:col-span-1">
                                <x-label for="office" class="font-semibold">Agency</x-label>
                                <multi-select :items="offices" v-slot="{ selected }" name="office" label="name"></multi-select>
                            </div>
                            ...
                            <div class="col-span-1 sm:col-span-1 my-auto">
                                <button class="bg-darkblue border border-transparent duration-150 ease-in-out focus:border-red-300 focus:outline-none focus:shadow-outline-red font-medium inline-flex items-center justify-center p-4 rounded-md sm:leading-5 sm:text-sm text-white transition w-40" @click="getSalesByAgencyPdfData">
                                    Search
                                </button>
                            </div>
