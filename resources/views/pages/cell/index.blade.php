                ...
                <x-table :headers="$headers">
                    <x-slot name="body">
                        <tr>
                            ...
                            <td class="text-center border-b-2 border-gray-300 px-3">
                                <input @input="setParam('icon', $event.target.value)" name="icon" class="form-input w-full mx-auto my-3 py-2 px-3 bg-gray-200 rounded shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out leading-none" />
                            </td>
                            <td colspan="2" class="text-center border-b-2 border-gray-300 px-3"></td>
                        </tr>
                        <template v-if="data.length > 0">
                            <tr v-for="(cell, key) in data" :key="key" class="hover:bg-gray-100 cursor-pointer">
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{cell.name}}
                                </td>
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    <img :src="cell.image_path" class="mx-auto w-6">
                                </td>

                                <td class="px-6 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium text-gray-500 flex py-5">
                                    <delete-button :url="cell.deleteUrl"></delete-button>
                                    <a :href="cell.showUrl" class="focus:outline-none focus:shadow-outline inline-flex">
