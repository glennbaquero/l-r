    ...
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        ...
        <div class="mt-12 mx-auto">
            <data-table v-slot="{ params, setParam, data, links, meta, next, prev, loading }"
                        ...
                        url="{{route('recommendation.fetch')}}"
            >
                <x-table :headers="$headers">
                    <x-slot name="body">
                        ...
                        <template v-if="data.length > 0">
                            <tr v-for="(recommendation, key) in data" :key="key" class="hover:bg-gray-100 cursor-pointer">
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{recommendation.name}}
                                </td>
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    <span :class="{ 'bg-red-500': !recommendation.from_youtube, 'bg-green-500': recommendation.from_youtube }" class="relative inline-block flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:shadow-outline" role="checkbox" tabindex="0">
                                        <span aria-hidden="true" :class="{'-ml-5': !recommendation.from_youtube, 'ml-5': recommendation.from_youtube}" class="inline-block h-5 w-5 rounded-full bg-white shadow transform transition ease-in-out duration-200"></span>
                                    </span>
                                </td>

                                <td class="px-6 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 font-medium text-gray-500 flex py-5">
                                    <delete-button :url="recommendation.deleteUrl"></delete-button>
                                    <a :href="recommendation.showUrl" class="focus:outline-none focus:shadow-outline inline-flex">
