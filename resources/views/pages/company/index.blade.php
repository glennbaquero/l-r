                ...
                <x-table :headers="$headers">
                    <x-slot name="body">
                        ...
                        <template v-if="data.length > 0">
                            <tr v-for="(company, key) in data" :key="key" class="hover:bg-gray-100 cursor-pointer">
                                ...
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{company.max_credit_line}}
                                </td>
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{company.max_number_ticket}}
                                </td>
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{company.discount}}
                                </td>
                               {{--  <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    <span :class="{ 'bg-red-500': !currency.default_currency, 'bg-green-500': currency.default_currency }" class="relative inline-block flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:shadow-outline" role="checkbox" tabindex="0">
                                        <span aria-hidden="true" :class="{'-ml-5': !currency.default_currency, 'ml-5': currency.default_currency}" class="inline-block h-5 w-5 rounded-full bg-white shadow transform transition ease-in-out duration-200"></span>
                                    </span>
                                </td> --}}

                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-500">
                                    <delete-button :url="company.deleteUrl"></delete-button>
                                    <a :href="company.showUrl" class="focus:outline-none focus:shadow-outline inline-flex">
