    ...
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        <div class="flex items-center">
            <div class="text-base mr-auto">
                ...
                <x-breadcrumb currentModule="{{__('Printer Management')}}" currentPage="Show" route="{{ route('printer.index') }}"> 
                            ...
                            @csrf
                                <div class="grid grid-cols-6 gap-6" >
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <x-select :lists="$brands" name="printer_brand_id" @change="selectChanged({{$brands}}, $event.target.value, 'printer')" :selected="$printer->printer_brand_id"/>
                                    </div>
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <select class="form-input w-full mx-auto my-3 bg-gray-200 rounded shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out leading-none border-transparent" name="printer_model_id">
                                            <option v-for="model in item.printer_models" :value="model.id" :selected="model.id === {{ $printer->printer_model_id }}">@{{ model.name }}</option>
                                        </select>
                                    </div>

                                    <div class="col-span-6 sm:col-span-3">
                                        <x-label for="name" class="font-semibold">Name</x-label>
                                        <x-form-input type="text" name="name" id="name" value="{{ $printer->name }}" />
                                    </div>

                                    <div class="col-span-6 sm:col-span-3">
                                        <x-label for="code" class="font-semibold">Code</x-label>
                                        <x-form-input type="text" name="code" id="code" value="{{ $printer->code }}" />
                                    </div>

                                    <div class="col-span-full sm:col-span-full">
                                        <x-label for="notes" class="font-semibold">Notes</x-label>
                                        <x-text-area name="notes" value="{{ $printer->notes }}"/>
                                    </div>
                                </div>
