    ...
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        <div class="flex items-center">
            <div class="text-base mr-auto">
                ...
                <x-breadcrumb currentModule="{{__('Terminal Management')}}" route="{{ route('terminal.index') }}"></x-breadcrumb>
            </div>
        </div>
                            ...
                            @csrf
                                <div class="grid grid-cols-6 gap-6" >
                                    ...
                                    <div class="col-span-2 sm:col-span-2">
                                        ...
                                        <x-select :lists="$operatingSystems" name="operating_system" identifierValue="name"/>
                                    </div>
                                    <div class="col-span-2 sm:col-span-2">
                                        <x-label for="web_browser" class="font-semibold">Web Browser</x-label>
                                        <x-select :lists="$webBrowsers" name="web_browser" identifierValue="name"/>
                                    </div>

                                    <div class="col-span-6 sm:col-span-3">
                                        <x-label for="printer_brand_id" class="font-semibold">Printer Brand</x-label>
                                        <x-select :lists="$printerBrands" name="printer_brand_id" @change="selectChanged({{$printerBrands}}, $event.target.value, 'printer')"/>
                                    </div>
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <select class="form-input w-full mx-auto my-3 bg-gray-200 rounded shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out leading-none border-transparent" name="printer_id">
                                            <option v-for="printer in item.printers" :value="printer.id">@{{ printer.name }}</option>
                                        </select>
                                    </div>
                                </div>
