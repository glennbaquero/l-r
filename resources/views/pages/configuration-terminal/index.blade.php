    ...
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        ...
        <div class="mt-12 mx-auto">
            <data-table v-slot="{ params, setParam, data, links, meta, next, prev, loading }"
                        ...
                        url="{{route('terminal.fetch')}}"
            >
                <x-table :headers="$headers">
                    <x-slot name="body">
                        <tr>
                            <td class="text-center border-b-2 border-gray-300 px-3">
                                <input @input="setParam('office', $event.target.value)" name="office" class="form-input w-full mx-auto my-3 py-2 px-3 bg-gray-200 rounded shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out leading-none" />
                            </td>
                        ...
                        <template v-if="data.length > 0">
                            {{-- <toggle v-slot="{ display, toggled }">
                                <div>
                                    <template  v-for="byOffice in data">
                                        <tr class="hover:bg-gray-100 cursor-pointer">
                                            <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                                @{{ byOffice[0].office_name }} - @{{ byOffice.length }} item(s)
                                            </td>
                                            <td colspan="3" class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900"></td>
                                        </tr>
                                        <template v-for="(terminal, key) in byOffice">
                                            <tr :key="key" class="hover:bg-gray-100 cursor-pointer">
                                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                                    @{{terminal.operating_system}}
                                                </td>
                                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                                    @{{terminal.web_browser}}
                                                </td>
                                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                                    @{{terminal.printer}}
                                                </td>
                                                ...
                                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-500">
                                                    <delete-button :url="terminal.deleteUrl"></delete-button>
                                                    <a :href="terminal.showUrl" class="focus:outline-none focus:shadow-outline inline-flex">
                            ...
                            <tr v-for="(terminal, key) in data" :key="key" class="hover:bg-gray-100 cursor-pointer">
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{terminal.office_name}}
                                </td>
                                <td class="px-6 py-2 whitespace-no-wrap text-center border-b border-gray-200 text-sm leading-5 font-medium text-gray-900">
                                    @{{terminal.operating_system}}
                                </td>
