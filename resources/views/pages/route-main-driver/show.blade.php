    ...
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        <div class="flex items-center">
            <div class="text-base mr-auto">
                ...
                <x-breadcrumb currentModule="{{__('Route and Main Driver')}}" currentPage="Show" route="{{ route('route-main-driver.index') }}"> 
        ...
        <div class="mt-12 mx-auto w-3/4">
            <div class="bg-white shadow sm:rounded-lg">
                <div class="px-4 py-5 sm:p-6">
                    <tab v-slot="{ selected, menuChanged }" default-selected="{{ $item->type }}">
                        <div>
                            ...
                            <form action="{{ route('route-main-driver.update', $item->id) }}" method="POST">
                                @csrf
                                ...
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-2 sm:col-span-2">
                                        ...
                                        <x-form-input type="text" name="type_of_record" value="{{ $item->type_of_record }}" />
                                    </div>
                                    ...
                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Voucher of Route'">
                                        ...
                                        <x-select :lists="$types" name="voucher_type" display="label" identifierValue="label" :selected="$item->voucher_type"/>
                                    </div>

                                    <div class="col-span-2 sm:col-span-2">
                                        <x-label for="series" class="font-semibold">Series</x-label>
                                        <x-form-input type="text" name="series" value="{{ $item->series }}" />
                                    </div>
                                    <div class="col-span-2 sm:col-span-2">
                                        <x-label for="correlative" class="font-semibold">Correlative</x-label>
                                        <x-form-input type="text" name="correlative" value="{{ $item->correlative }}" />
                                    </div>
                                    ...
                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        ...
                                        <x-form-input type="text" name="first_name" value="{{ $item->first_name }}" />
                                    </div>
                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="last_name" class="font-semibold">Last Name</x-label>
                                        <x-form-input type="text" name="last_name" value="{{ $item->last_name }}" />
                                    </div>

                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="driver_id" class="font-semibold">Driver</x-label>
                                        <x-select :lists="$drivers" name="driver_id" display="fullname" :selected="$item->driver_id"/>
                                    </div>

                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="departure_id" class="font-semibold">Departure</x-label>
                                        <x-select :lists="$cities" name="departure_id" :selected="$item->departure_id" />
                                    </div>

                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="arrival_id" class="font-semibold">Arrival</x-label>
                                        <x-select :lists="$cities" name="arrival_id" :selected="$item->departure_id"/>
                                    </div>

                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="travel_date" class="font-semibold">Travel Date</x-label>
                                        <x-datepicker name="travel_date" :item="$item"/>
                                    </div>

                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="time" class="font-semibold">Time</x-label>
                                        <x-datepicker name="time" type="time" format="hh:mm" :item="$item"/>
                                    </div>

                                    <div class="col-span-2 sm:col-span-2">
                                        <x-label for="issued_date" class="font-semibold">Issued Date</x-label>
                                        <x-datepicker name="issued_date" :item="$item"/>
                                    </div>

                                    <div class="col-span-2 sm:col-span-2" v-if="selected == 'Ticket of Route'">
                                        <x-label for="seat" class="font-semibold">Seat</x-label>
                                        <x-form-input type="text" name="seat" value="{{ $item->seat }}" />
                                    </div>
                                    ...
                                    <div class="col-span-2 sm:col-span-2"  v-if="selected == 'Ticket of Route'">
                                        ...
                                        <select class="form-input w-full mx-auto my-3 bg-gray-200 rounded shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out leading-none border-transparent" name="state">
                                            ...
                                            <option value="Sold" {{ $item->state === 'Sold' ? 'selected' : '' }} >Sold</option>
                                            <option value="Cancelled" {{ $item->state === 'Cancelled' ? 'selected' : '' }} >Cancelled</option>
                                            <option value="To collect" {{ $item->state === 'To collect' ? 'selected' : '' }} >To collect</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-span-2 sm:col-span-2"  v-if="selected != 'Ticket of Route'">
                                        ...
                                        <select class="form-input w-full mx-auto my-3 bg-gray-200 rounded shadow-sm focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out leading-none border-transparent" name="state">
                                            <option disabled selected>Select your option</option>
                                            <option value="Registered" {{ $item->state === 'Registered' ? 'selected' : '' }}>Registered</option>
                                            <option value="Cancelled" {{ $item->state === 'Cancelled' ? 'selected' : '' }}>Cancelled</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="col-span-2 sm:col-span-2">
                                        ...
                                        <x-form-input type="number" min="0" step="any" name="price" value="{{ $item->price }}" />
                                    </div>

                                    <div class="col-span-2 sm:col-span-2">
                                        <x-label for="code_reference" class="font-semibold">Code Reference</x-label>
                                        <x-form-input type="text" name="code_reference" value="{{ $item->code_reference }}" />
                                    </div>
